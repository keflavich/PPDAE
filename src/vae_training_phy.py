...
class Trainer():
    ...
    def __init__(self, model, optimizer, batch_size, wandb,
        ...
        self.train_loss = {'Loss': [], 'BCE': [], 'KLD': []}
        self.test_loss = {'Loss': [], 'BCE': [], 'KLD': []}
        self.num_steps = 0
    ...
    def _loss(self, x, xhat, mu, logvar, train=True, ep=0):
        ...
        loss
            loss value
        """
        rec = self.bce_loss(xhat, x)
        kld_l = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        loss = rec + self._beta_scheduler(ep) * kld_l

        if train:
            self.train_loss['BCE'].append(rec.item())
            self.train_loss['KLD'].append(kld_l.item())
            self.train_loss['Loss'].append(loss.item())
        else:
            self.test_loss['BCE'].append(rec.item())
            self.test_loss['KLD'].append(kld_l.item())
